...
import 'package:lumo_ai_travel_plan_app/widget/elevated_button_widget.dart';
...
class _AttractionPageState extends State<AttractionPage> {
  ...
  Future<void> _initFetchPlaces() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('無法取得定位或地點資訊：$e')),
      );
    }
  }
  ...
  Future<void> _refreshLocationAndPlaces() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('✅ 已重新定位並更新附近景點')),
      );
    } catch (e) {
      print("重新定位失敗: $e");
      ScaffoldMessenger.of(context).showSnackBar(
