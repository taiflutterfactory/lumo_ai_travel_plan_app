...
class FoodDetailPage extends StatefulWidget {
  ...
  const FoodDetailPage({super.key, required this.placeId});
...
class _FoodDetailPageState extends State<FoodDetailPage> {
  ...
  Future<void> _fetchFoodDetail() async {
    try {
      ...
      final detailData = await mapService.fetchNearbyFoodDetail(widget.placeId, apiKey);

      setState(() {
        foodDetail = detailData.result;
        isLoading = false;
      });
    } catch (e) {
      print('取得詳細資料失敗: $e');
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => RoutePlannerPage(
          destination: LatLng(
          foodDetail!.geometry!.location.lat,
            foodDetail!.geometry!.location.lng
          ),
        ),
      ),
    );
  ...
  void initState() {
    ...
    _timer = Timer.periodic(const Duration(seconds: 3), (Timer timer) {
      if (!mounted) return;
      if (foodDetail?.photos == null || foodDetail!.photos!.isEmpty) return;

      if (_currentPage < foodDetail!.photos!.length - 1) {
        _currentPage++;
      } else {
        _currentPage = 0;
      }
  ...
  Widget build(BuildContext context) {
    if (isLoading || foodDetail == null) {
      return const Scaffold(
        body: Center(child: CircularProgressIndicator()),
      );
    }

    final photos = foodDetail?.photos ?? [];
    final openingHours = foodDetail?.opening_hours?.weekday_text ?? [];
    final website = foodDetail?.website;
    final phoneNumber = foodDetail?.formatted_phone_number;
    final rating = foodDetail?.rating;
    final reviews = foodDetail?.reviews ?? [];
      ...
      body: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  Row(
                    children: [
                      Text(foodDetail!.name, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold),),
                  ...
                  Text(foodDetail!.formatted_address!, style: const TextStyle(fontSize: 16,)),
                  const SizedBox(height: 8,),
            ...
            // ✏️ 景點介紹
            if (foodDetail?.editorial_summary?.overview != null)
              Padding(
                padding: const EdgeInsets.all(16.0),
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    sectionTitle('景點介紹'),
                    const SizedBox(height: 8),
                    Text(foodDetail!.editorial_summary!.overview),
                  ],
                ),
              ),
